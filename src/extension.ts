...
export function activate(context: vscode.ExtensionContext) {
    ...
    const disposable = vscode.commands.registerCommand(commandId, async () => {
        ...
        if (!selected) {
            vscode.window.showErrorMessage("Cannot create Empty Bin.");
            return;
        }
        ...
        vscode.window.showInformationMessage(message, ...[openURL, copyURL]).then(selection => {
            if (selection === openURL) {
                ...
                try {
                    copy(url);
                    vscode.window.showInformationMessage(`Successfully copied the URL to your clipboard.`);
                } catch (e) {
                    vscode.window.showErrorMessage(`Failed to copy the url. ${e}`);
                }
            }
        });
    });
